...
-- 댓글을 가장 많이 작성한 유저는?
-- select count() from comments group by user_id;
-- 사진을 게시한 유저는 모두 몇명?
select count(distinct photos_id) from comments where photos_id;
-- 3번 댓글이 달린 사진은?
select photos_id from comments where user_id=3;
-- 3번 댓글을 달린 사람 닉네임은?
select users.nickname from users,comments where users.nickname=comments.user_id, user_id=3 group by users.nickname;
